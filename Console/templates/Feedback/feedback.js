...
class FeedbackWindow extends UserDataComponent {
    onconnect() {
        ...
        this.els.commentsButton.addEventListener('click', () => {
            this.commentsToggled = !this.commentsToggled;
            this.els.commentsButton.textContent = this.commentsToggled ? 'Show comments' : 'Hide comments';
            if (this.commentsToggled) {
                this.els.comments.style.display = 'none';
            }
            else {
                this.els.comments.style.display = 'block';
            }
        })
...
SvgPlus.defineHTMLElement(FeedbackWindow);